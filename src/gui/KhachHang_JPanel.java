	...
	public void actionPerformed(ActionEvent e) {
		...
		if (o.equals(btnThem)) {
			if (validateKhachHang()) {
				...
				String sDT = txtSDT.getText();
				String email = txtEmail.getText();
				List<KhachHang> lskh = kh_dao.getKhachHang();
				boolean sdt= true;
				for (KhachHang a : lskh) {
					if(a.getsDT().equals(sDT))
						sdt=false;
				}
				
				//Tạo KH muốn thêm vào CSDL
				KhachHang kh = new KhachHang(maKhachHang, tenKhachHang, diaChi, sDT, email);
				if (kh_dao.themKhachHang(kh) && (sdt ==true) ) {
					xoaDL();
					...
					JOptionPane.showMessageDialog(null, " KHONG DUOC TRUNG MA HOAC SDT !");
				}
			}
		}
		// Button trang cuối
		...
		if (o.equals(btnXoa)) {
			...
			if (row == -1) {
				...
				// kiểm tra xem có tồn tại hóa đơn nào đã được tạo với khách hàng có mã 
				//đó hay không nếu không tìm thấy thì có thể xóa thông tin khách hàng đó mà không ảnh hưởng đến các hóa đơn khác
