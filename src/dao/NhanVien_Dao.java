...
public class NhanVien_Dao {
	public List<NhanVien> getNhanVien() {
		...
		try {
			...
			while (rs.next()) {
				list.add(new NhanVien(rs.getString("maNhanVien"), rs.getString("tenNhanVien"), rs.getString("chucVu"),
						rs.getDouble("luong"), rs.getString("diaChi"), rs.getBoolean("gioiTinh"), rs.getString("sDT"),
						rs.getString("cMND"), rs.getString("matKhau")));
			}
	...
	public boolean CapNhatNV(NhanVien nv) {
		...
		try {
			PreparedStatement statement = con.prepareStatement(
					"update NhanVien set tenNhanVien=?,chucVu=?,luong=?,diaChi=?,gioiTinh=?,sDT=?,cMND=?,matKhau=? where maNhanVien=?");
