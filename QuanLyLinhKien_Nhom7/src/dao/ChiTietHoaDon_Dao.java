	...
	public List<ChiTietHoaDon> getCTHDTheoMa(String ma) {
		...
		try {
			PreparedStatement statement = con.prepareStatement(
					"select *from ChiTietHoaDon join LinhKien on ChiTietHoaDon.maLinhKien=LinhKien.maLinhKien where maHoaDon=?");
			statement.setString(1, ma);
			ResultSet rs = statement.executeQuery();
			while (rs.next()) {
				list.add(new ChiTietHoaDon(
						new LinhKien(new NhaCungCap(), rs.getString("maLinhKien"), rs.getString("tenLinhKien"), null, 0,
								rs.getDouble("giaBan"), rs.getInt("baoHanh"), null, rs.getDate("ngayNhap")),
						new HoaDon(rs.getString("maHoaDon")), rs.getInt("soLuong"), rs.getInt("mucGiamGia")));
			}
		} catch (SQLException e) {
